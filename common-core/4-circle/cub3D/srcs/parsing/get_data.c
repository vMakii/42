...
/*   Updated: 2025/07/24 18:31:14 by gburtin          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
bool	get_color_2(int *texture, char *str)
{
	...
	if (color[0] > 255 || color[1] > 255 || color[2] > 255)
		return (false);
	*texture = RGB(color[0], color[1], color[2]);
	return (true);
}
...
bool	get_color(t_data *data, char *id)
{
	...
	{
		if (!ft_strncmp(data->map.tab[i], "F", 1) && ft_strncmp(id, "F", 1))
			if (get_color_2(&data->texture.floor_color,
				data->map.tab[i]) == true)
				return (true);
		if (!ft_strncmp(data->map.tab[i], "C", 1) && ft_strncmp(id, "C", 1))
...
bool	get_textures(t_data *data)
{
	...
	while (data->map.tab[i])
	{
		if (!ft_strncmp(data->map.tab[i], "NO", 2))
			if (get_texture_2(data, &data->texture.north_texture,
					data->map.tab[i]) == false)
				return (false);
		if (!ft_strncmp(data->map.tab[i], "SO", 2))
			if (get_texture_2(data, &data->texture.south_texture,
					data->map.tab[i]) == false)
				return (false);
		if (!ft_strncmp(data->map.tab[i], "WE", 2))
			if (get_texture_2(data, &data->texture.west_texture,
					data->map.tab[i]) == false)
				return (false);
		if (!ft_strncmp(data->map.tab[i], "EA", 2))
